...
namespace SuperiorHackBase.Core.ProcessInteraction.Memory
{
    public class ProcessMemoryException : Exception
    {
        ...
        public ProcessMemoryException(Pointer address, int processed, int count, int errorCode, bool result) : base(CreateMessage(address, processed, count, errorCode, result), new Win32Exception(errorCode))
        {
            Address = address;
            Processed = processed;
            Count = count;
