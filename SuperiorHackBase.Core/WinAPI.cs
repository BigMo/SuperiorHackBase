...
namespace SuperiorHackBase.Core
{
    ...
    public class WinAPI
    {
        ...
        public static IEnumerable<WinAPI.MEMORY_BASIC_INFORMATION> EnumeratePages(IntPtr handle)
        {
            SystemInfo sysInfo;
            GetSystemInfo(out sysInfo);
            MEMORY_BASIC_INFORMATION info;
            var address = (Pointer)sysInfo.MinimumApplicationAddress;
            while (true)
            {
                var numBytes = VirtualQueryEx(handle, address, out info, (uint)SizeCache<MEMORY_BASIC_INFORMATION>.Size);
                if (numBytes != SizeCache<MEMORY_BASIC_INFORMATION>.Size)
                    break;
                if (address > sysInfo.MaximumApplicationAddress)
                    break;
